...
{
    echo "<script>alert('Fill all field');window.location='setpapereti.html'</script>";
...
{
   $stmt=$conn->prepare("insert into etique(Question,OptionA,OptionB,OptionC,OptionD,Answer) values(?,?,?,?,?,?)");
    $stmt->bind_param("ssssss",$question,$optionA,$optionB,$optionC,$optionD,$answer);
    $stmt->execute();
    echo "<script>alert('Question Added successfully');window.location='setpapereti.html'</script>";
    $stmt->close();
    $conn->close();
}
?>